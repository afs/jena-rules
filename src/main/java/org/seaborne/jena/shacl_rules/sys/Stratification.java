...
public class Stratification {
        ...
        private static Stratification createStratification(Integer dataStratum, RuleSet ruleSet, DependencyGraph depGraph) {
            ...
            int maxStratumAssigned = 0;
            // There are no gaps (strata with zero rules) so if every stratum is
            // filled with one rule, there can be less that this many strata:
            ...
            while(changed) {
                ...
                for ( Edge e : depGraph.edges() ) {
                    ...
                    switch(e.link()) {
                        ...
                        case NEGATIVE -> {
                            if ( stratumMap.get(pRule) <= stratumMap.get(qRule) ) {
                                int x = 1+ stratumMap.get(qRule);
                                maxStratumAssigned = Math.max(maxStratumAssigned, x);
                                if ( x > limit )
                                    throw new RulesException("Stratification error");
                                stratumMap.put(pRule, x);
                                changed = true;
